...
=head1 SYNOPSIS
 generate_nanopore_fastq_pipeline_makefile [options]
  ...
  -m     make file name
=head1 DESCRIPTION
This script implements the pipeline for Guppy base calling of nanopore data.
=cut
...
#programs
my $guppy_base_caller = "/usr/local/ont-guppy-cpu/bin/guppy_basecaller";
my $guppy_barcoder = "/usr/local/ont-guppy-cpu/bin/guppy_barcoder";
...
guppy_basecaller -i INPUT_FAST5_DIR -r -s OUTPUT_DIR --flowcell FLOWCELL --kit KIT -x auto

/usr/local/ont-guppy-4.5.4/bin/guppy_basecaller -i fast5 -r -s guppy_output --flowcell FLO-MIN106 --kit SQK-LSK109 --num_callers 4 --cpu_threads_per_caller 2
#################
#Read sample file 
#################
my %SAMPLE = ();
